...
s2.n1 = NA #initialized vector for S_{n-1}^2
s2.n = NA #initialized vector for S_{n}^2
n = 20
var.true = 2
...
for(i in 1:1000){
  ...
  s2.n1[i] = sum(sqr.dev) / (n-1)
  s2.n[i] = sum(sqr.dev) / n
}

# Compare estimators:
(bias.n1 = mean(s2.n1 - var.true))
(bias.n = mean(s2.n - var.true))

(mse.n1 = mean((s2.n1 - var.true)^2))
(mse.n = mean((s2.n - var.true)^2))
```
